...
def welcome_message():
    ...
    print("\n" + bcolors.BOLD +
          ...
          bcolors.ENDC)
    print("\n" + bcolors.BOLD +
          center_text("The aim of the game is to hit your opponents\
           ships in the least trys possible!") +
          bcolors.ENDC)
    print("\n" + bcolors.BOLD +
          center_text("Just select a gameboard size between a 5x5\
           grid and a 9x9 grid!") +
          bcolors.ENDC)
    print("\n" + bcolors.BOLD +
          center_text("Then enter the co-ordinates of where you\
           want to shoot when prompted!") +
          bcolors.ENDC)
...
def display_high_scores():
    ...
    """
    print("\n" + bcolors.HEADER +
          center_text("Current High Scores") +
          bcolors.ENDC)
    print("\n" + bcolors.OKCYAN +
          center_text("Top 5 Lowest Misses:") +
          bcolors.ENDC)
...
def display_boards(player_board, computer_board):
    ...
    for i in range(board_size):
        ...
        row_str = f"{bcolors.OKCYAN}{letters[i]: <2}\
        {bcolors.ENDC}\
        {player_row_str: <{len(player_row_str) + 2}}\
        | {bcolors.OKCYAN}{letters[i]: <2}\
        {bcolors.ENDC}\
...
def get_target(board):
    ...
    while True:
        try:
            ...
            print("Invalid input. Please enter letters for Rows\
    ...
    while True:
        ...
        if computer_board[row][col] == 'M' or computer_board[row][col] == 'X':
            ...
            if (row, col) in computer_ships:
                ...
                print(bcolors.OKGREEN + "Hit! Target: " +
                      chr(ord('A') + row) +
                      " " +
                      str(col + 1) +
                      bcolors.ENDC)
...
def save_to_high_scores(misses):
    ...
    while True:
        save_score = input(
            "Do you want to save your score to the high scores? (y/n): ")
